"""Interactive dashboard: monthly Zambia climate & vegetation data on a map.
...
from dashboard_data import PARQUET_CACHE, VARIABLES, load_combined, district_monthly_averages

st.set_page_config(
    page_title="Zambia Climate & Vegetation Dashboard",
    layout="wide",
)

st.title("Zambia Monthly Climate & Vegetation Dashboard")
...
load_combined_cached = st.cache_data(show_spinner="Loading Zambia monthly dataset...")(
    load_combined
)
...
def load_geojson():
    geojson_path = pathlib.Path("zambia_monthly_ingest/districts.geojson")
...
district_df = district_monthly_averages(df)
...
with st.sidebar:
    ...
    resolution = st.radio("Resolution", ["5 km Grid", "District Averages"])
...
if animate:
    if resolution == "5 km Grid":
        ...
        district_month_df = district_df.copy()
        district_month_df['month_str'] = district_month_df['month'].dt.strftime("%Y-%m")
        # Ensure it's sorted by date for animation
        district_month_df = district_month_df.sort_values("month")
    ...
    month_df = df[df["month"] == selected_month]
    district_month_df = district_df[district_df["month"] == selected_month]
...
with tab_climate:
    ...
    if resolution == "5 km Grid":
        ...
        col1.metric("National Mean", f"{district_month_df[variable].mean():.2f} {meta['unit']}".strip())
        col2.metric("Lowest District", f"{district_month_df[variable].min():.2f} {meta['unit']}".strip())
        col3.metric("Highest District", f"{district_month_df[variable].max():.2f} {meta['unit']}".strip())
        col4.metric("Valid Counties", f"{len(district_month_df):,}")
    ...
    if resolution == "5 km Grid":
        map_kwargs = dict(
            ...
            mapbox_center={"lat": -13.1, "lon": 27.8},
            zoom=4.5,
        ...
        if districts_geojson is None:
            st.error("District boundary GeoJSON not found. Please run the mapping script.")
    ...
    if resolution == "5 km Grid":
        ...
        if selected_districts:
            trend_df = district_df[district_df["district"].isin(selected_districts)]
            trend_fig = px.line(
                trend_df,
                x="month",
                y=variable,
            ...
            summary = district_df.groupby("month")[variable].mean().reset_index()
            trend_fig = px.line(
                summary,
                x="month",
                y=variable,
    ...
    with dl_col1:
        ...
        if PARQUET_CACHE.exists():
            st.download_button(
                ...
                _read_parquet_bytes(),
                file_name="zambia_climate_vegetation_all_months.parquet",
    ...
    with dl_col2:
        ...
        st.link_button(
            ...
            "https://github.com/Masuzyo/zambia-climate-dashboard/archive/refs/heads/master.zip",
        )

    with dl_col3:
        st.markdown("**District averages (CSV)**")
        st.caption("One row per month per district.")
        st.download_button(
            "Download district averages CSV",
            district_df.to_csv(index=False),
            file_name="zambia_monthly_district_averages.csv",
            mime="text/csv",
        )

    with dl_col4:
        st.markdown("**National averages (CSV)**")
        st.caption("One row per month: Zambia-wide mean.")
        st.download_button(
            "Download national averages CSV",
            _national_monthly_averages(df).to_csv(index=False),
            file_name="zambia_monthly_national_averages.csv",
            mime="text/csv",
        )
    ...
    with st.expander("Raw data (selected month)"):
        ...
        st.download_button(
            ...
            file_name=f"zambia_{month_labels[month_idx]}_{variable}.csv",
            mime="text/csv",
        )
...
with tab_malaria:
    ...
    if pdf is not None:
        ...
        st.subheader("HbAS (Sickle Trait) Prevalence by District")
        district_res = pdf.groupby('district')['is_mutant'].mean().reset_index()
        district_res['Prevalence Rate (%)'] = district_res['is_mutant'] * 100
        ...
        with dem_col3:
            if 'mean_temp_c' in district_df.columns and 'elevation_m' in district_df.columns:
                baseline = district_df.groupby('district')[['mean_temp_c', 'rain_mm', 'elevation_m']].mean().reset_index()
                res_climate = district_res.merge(baseline, on='district')
        ...
        st.markdown("---")
        st.subheader("District Population & Resistance Breakdown")
        district_table = pdf.groupby('district').agg(
            Total_Patients=('patient_id', 'count'),
            HbAS_Cases=('is_mutant', 'sum')
        ).reset_index()
        district_table['HbAS_Prevalence_%'] = (district_table['HbAS_Cases'] / district_table['Total_Patients']) * 100
        
        st.dataframe(
            district_table.sort_values('Total_Patients', ascending=False),
            column_config={
                "district": "District",
...
with tab_modeling:
    ...
    if pdf is not None and not district_df.empty:
        ...
        available_predictors = [p for p in predictors if p in district_df.columns]
        
        baseline = district_df.groupby('district')[available_predictors].mean().reset_index()
        
        # 3. Merge
        model_df = district_table.merge(baseline, on='district')
